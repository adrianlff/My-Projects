...
```
┌─────────────┐     ┌──────────────────┐     ┌─────────────────────┐
│   Apify      │────▶│  Make (orquesta) │────▶│   CRM propio         │
...
- **Make**: orquesta todo el flujo (dispara scraping, llama a Claude para el análisis, envía emails, agenda citas). Es el "pegamento" entre servicios.
...
2. **Scraping (Apify)**: Make llama al actor de Apify (Google Maps Scraper) con esos parámetros. Devuelve: nombre, dirección, teléfono, web (si existe), rating, nº reseñas, categoría.
3. **Ingesta**: Make inserta cada resultado en el CRM (Supabase) como lead en estado `Nuevo`.
4. **Análisis IA**:
...
9. **Web propia + Agente 24/7**: en paralelo, la web de la agencia capta leads propios (formulario + chat del agente), que entran directamente al mismo CRM. El agente (Claude + WhatsApp/web widget) responde dudas, cualifica y agenda citas en Google Calendar de forma autónoma.
...
|---|---|
| **Make** (`mcp__33f327a3...`) | Crear y gestionar los escenarios de automatización: scraping, análisis, envío de propuestas, agente. Aquí es donde se conectan Apify, Gmail, Google Calendar, WhatsApp y Anthropic como apps dentro de un mismo flujo. |
...
| **GitHub** | Repositorio del CRM (frontend Next.js) y de la web de captación. |
...
### A conectar (vía Make, o como conectores directos de Claude si se prefiere operar el CRM desde el chat)

| Conector | Para qué | Dónde se contrata/conecta |
|---|---|---|
| **Apify** | Actor "Google Maps Scraper" (datos de negocios) + "Website Content Crawler" (contenido de webs existentes) | Cuenta Apify (plan ~$49/mes), conectado como app dentro de Make |
| **Anthropic (Claude API)** | Scoring, generación de propuestas, agente conversacional 24/7 | API key de Anthropic, llamada vía módulo HTTP en Make o desde una Edge Function de Supabase |
| **Gmail / Google Workspace** | Envío automático de emails con propuesta adjunta | Conexión OAuth dentro de Make (o conector Gmail de Claude si se gestiona desde el chat) |
| **Google Calendar** | Agendar citas que cualifica el agente 24/7 | Conexión OAuth dentro de Make |
| **WhatsApp Business API** (vía Twilio o 360dialog) | Canal del agente 24/7 para las clínicas y para tus propios leads | Cuenta de WhatsApp Business API, conectada como app en Make |
| **Stripe** (opcional, fase posterior) | Cobro de setup/mantenimiento a clientes cerrados | Conexión directa o vía Make |

**Nota práctica**: Make es el punto de integración recomendado para Apify/Gmail/Calendar/WhatsApp porque estos servicios necesitan ejecutarse 24/7 de forma autónoma (el CRM y el agente deben funcionar aunque no haya una sesión de Claude abierta). Los conectores MCP directos en Claude son útiles para que tú operes el CRM conversacionalmente (ej. "lánzame el scraping de clínicas en Sevilla"), pero la ejecución recurrente/automática vive en Make + Supabase.
...
Cada transición de estado se implementa como: **trigger** (cambio de estado detectado vía webhook de base de datos en Supabase) → **escenario de Make correspondiente** → **acción** (email, calendario, notificación, generación de documento).
...
| **Agente IA 24/7** | Chat en web + WhatsApp que responde dudas, cualifica interesados y agenda citas en su calendario | 300 – 500 € | 80 – 150 €/mes (incluye coste de API de Claude y WhatsApp Business) | Cualquier clínica con volumen de consultas |
...
- Make: ~9-29 €/mes según volumen de operaciones.
...
- WhatsApp Business API: variable por conversación (vía Twilio/360dialog), estimar 0,01-0,05 €/conversación iniciada por el negocio.

Con esto, tu coste fijo mensual del sistema ronda 100-150 €/mes para cientos de leads cualificados automáticamente — el margen sobre cualquier cliente cerrado es alto.
...
1. Modelo de datos en Supabase (tabla `leads` con estado, score, propuesta, timeline) + escenario de Make que hace scraping + inserta leads `Nuevo`.
2. Escenario de análisis IA (Claude) que cualifica y genera la propuesta.
...
5. Web propia + agente 24/7 (última fase, una vez el pipeline de captación ya genera leads cualificados).

Dime si quieres que empecemos por el modelo de datos de Supabase y el primer escenario de Make (scraping → CRM), o prefieres primero el panel del CRM.
